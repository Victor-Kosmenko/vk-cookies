...
( function( $) {
    $.wpMediaUploader = function( options ) {
        ...
        $( settings.target ).append('<div><img src="#" style="display: none; width: ' + settings.previewSize + '"/></div>')
        
        $( settings.buttonClass ).css( settings.buttonStyle );
        
        if($(settings.target).find('input').val()){
            $(settings.target).find('img').attr( 'src', $(settings.target).find('input').val()).show();
        }
